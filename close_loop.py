...
DEBUG = False
# Iterative Descent Configuration
# -----------------------------------------
ITERATIVE_DESCENT = True  # Enable multi-iteration refinement
MAX_DESCENT_ITERATIONS = 3  # Maximum number of descent iterations
DESCENT_PERCENTAGE = 0.6  # Descend 60% each iteration (can be 0.5-0.7)
MIN_HEIGHT_FOR_ITERATION = 10  # Minimum height to continue iterating (meters)
...
def main_pipeline():
    ...
    for i in range(0,iterations):    
        ...
        # Main pipeline
            ...
            request_counter = 0
            # Initialize descent iteration counter
            descent_iteration = 0
            max_iterations = MAX_DESCENT_ITERATIONS if ITERATIVE_DESCENT else 1
            
            while abs(curr_height) > 15 and descent_iteration < max_iterations:
                print(f"\n--- DESCENT ITERATION {descent_iteration + 1}/{max_iterations} ---")
                print(f"Current height: {curr_height:.2f}m")
                ...
                # save image with iteration info
                cv2.imwrite(f"images/mono_iter{i}_descent{descent_iteration}_req{request_counter}.jpg", cv2.cvtColor(np_arr,cv2.COLOR_RGB2BGR))
                # surface crop
                bounding_boxes = None
                depth_map = None
                ...
                # Determine if this is the final iteration or intermediate descent
                is_final_iteration = (descent_iteration >= max_iterations - 1) or (curr_height < MIN_HEIGHT_FOR_ITERATION)
                ...
                if ONLY_CROP_PIPELINE:
                    ...
                    # Iterative descent logic
                    if ITERATIVE_DESCENT and not is_final_iteration:
                        # Intermediate descent: move to X,Y and descend partially
                        print(f"Intermediate descent: Moving to ({tx:.2f}, {ty:.2f}) and descending {DESCENT_PERCENTAGE*100:.0f}%")
                        
                        # Move to X,Y at current altitude first
                        drone.client.moveToPositionAsync(tx, ty, pose.z_val, 3).join(); time.sleep(3)
                        
                        # Partial descent 
                        current_altitude = abs(drone.client.getMultirotorState().kinematics_estimated.position.z_val)
                        descent_amount = curr_height * DESCENT_PERCENTAGE
                        new_altitude = -(current_altitude - descent_amount)  # AirSim uses negative Z
                        
                        print(f"Descending from {current_altitude:.2f}m to {abs(new_altitude):.2f}m")
                        drone.client.moveToZAsync(new_altitude, 2).join(); time.sleep(3)
                        
                        # Update current height for next iteration
                        curr_height = drone.get_rangefinder()
                        print(f"After descent, height to ground: {curr_height:.2f}m")
                        
                    else:
                        # Final iteration: complete landing sequence
                        print(f"Final iteration: Complete landing to target")
                        curr_height = drone.move_drone(tx,ty,tz)
                # saving test results
                if test:
                    cv2.imwrite(f"tests/mono_depth{request_counter}_it_{i}_descent{descent_iteration}.jpg", cv2.cvtColor(np_arr,cv2.COLOR_RGB2BGR))
                    select_pil_image.save(f"tests/selected_depth{request_counter}_it_{i}_descent{descent_iteration}.jpg")
                    start_data_rec("depth",i,request_counter,ans)
                request_counter+=1
                descent_iteration += 1  # Increment descent iteration counter
                
                # If this was an intermediate descent, continue the loop
                # If final iteration or landing completed, the while condition will exit
                # clearing data    
                if DEBUG:
                    input("To continue and delete images, press enter")
                
            # Print iteration summary
            if ITERATIVE_DESCENT:
                print(f"\n=== ITERATIVE DESCENT SUMMARY ===")
                print(f"Total descent iterations completed: {descent_iteration}")
                print(f"Final height to ground: {curr_height:.2f}m")
                print(f"Reason for completion: {'Height < 15m' if abs(curr_height) <= 15 else 'Max iterations reached'}")
